...
readonly ingester_password="integration-ingester-only"
...
export THE_SEARCH_DATABASE_INGESTER_PASSWORD="${ingester_password}"

docker compose --project-name "${project_name}" up --detach --wait --wait-timeout 120 database
docker compose --project-name "${project_name}" run --rm database-roles
...
export TEST_INGESTER_DATABASE_URL="postgres://the_search_ingester:${ingester_password}@127.0.0.1:${database_port}/the_search?sslmode=disable"
